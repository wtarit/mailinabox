...
app_python_minor=$(
	"$MIAB_APP_PYTHON" -c 'import sys; print(f"{sys.version_info[0]}.{sys.version_info[1]}")'
)
...
if [ ! -x "$duplicity_venv/bin/python" ] || ! "$duplicity_venv/bin/python" -c \
	...
	hide_output "$MIAB_UV" venv "$duplicity_venv" --python "$MIAB_APP_PYTHON"
fi
